# CLAUDE.md — LevelUp.ai Project Context

## What This Is
LevelUp.ai — AI-powered networking automation. Analyzes WhatsApp chats + LinkedIn profiles to generate personalized talking points, key facts, action items, and follow-up messages in the user's voice. Handles scheduling. 10x networking by automating the boring parts.
...
n8n Workflow (levelup-ai-workflow.json)
...
- `frontend/LevelUpAI.jsx` — Alternate UI (8-screen onboarding flow)
...
- `workflows/levelup-ai-workflow.json` — n8n workflow (Webhook → Proxycurl → Claude → Supabase → Response)
